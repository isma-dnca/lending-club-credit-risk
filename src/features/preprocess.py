...
def engineer_emp_length(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert `emp_length` from text categories into a numeric feature.

    The original `emp_length` column is dropped after transformation.
    ...
    -------
    pd.DataFrame
        DataFrame with `emp_length_num` added.
    """
    df = df.copy()
...
def engineer_ratio_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Create simple ratio-based features from existing numeric columns.

    Features created when source columns exist:
    - loan_to_revenue
    - loan_to_fico
    - revenue_to_fico
